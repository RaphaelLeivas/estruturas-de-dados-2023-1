...
void Ins(Registro Reg, Apontador Ap, short *Cresceu, Registro *RegRetorno,
         Apontador *ApRetorno) {
    long i = 1;
    long j;
    Apontador ApTemp;
    ...
    if (Reg.Chave == Ap->r[i - 1].Chave) {  // printf(" Erro: Registro ja esta
                                            // presente %ld\n",Reg.Chave); i
        *Cresceu = FALSE;
        return;
    }
...

{
    ...
    nivel++;
    for (i = 0; i <= p->n; i++) ImprimeI(p->p[i], nivel, out, st, pref);
}
